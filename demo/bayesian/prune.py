...
def main():
    ...
    original_laplace_curv: Optional[laplace.ParametricLaplace]
    laplace_curv_path: Optional[Path]
    original_laplace_curv, laplace_curv_path = checkpoint.load_object(laplace_filename, path_args={"save_path": laplace_log_path}, library='dill', with_path=True)
    if original_laplace_curv is None:
    ...
    utils.verbose_and_log(f"Laplace loaded to model: {original_laplace_curv.model}", args.verbose, args.log)
    laplace_curv: laplace.ParametricLaplace = copy.deepcopy(original_laplace_curv)
