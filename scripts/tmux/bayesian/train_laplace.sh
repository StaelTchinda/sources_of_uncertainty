...
output_file="$PROJECT_PATH/verbose/${data}/${model}/bayesian/laplace/run_$(date +%Y%m%d_%H%M%S).txt"
...
tmux new -As $session_name -d "$PROJECT_PATH/venv/bin/python -O $PROJECT_PATH/demo/bayesian/train_laplace.py --data $data --model $model > >(tee -a $output_file) 2> >(tee -a $output_file >&2);"
