...
from config.data.lightning import iris as iris_config, mnist as mnist_config, ambiguous_mnist as ambiguous_mnist_config, cifar as cifar_config, cifar_c as cifar_c_config, wildcam as wildcam_config, imagenet as imagenet_config
...
def get_default_joint_datamodule(data_mode: JointDataMode) -> pl.LightningDataModule:
    ...
    elif data_mode == "cifar10_c_fog":
        return cifar_c_config.get_default_datamodule(data_mode)
    else:
        raise ValueError(f"Unknown data mode {data_mode}")
